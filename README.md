...
[GitHub Actions]
   ...
   POST /api/batch/trigger
        ↓
[RssFetchService]
   RSS 파싱 → URL 중복 체크 → AI 요약/태그 추출 → 태그 중복 체크 → DB 저장 → Discord 알림
```
...
| POST | `/api/batch/trigger` | RSS 수집 수동 실행 (X-Batch-Secret 헤더 필요) |

## GitHub Actions

- **batch-trigger.yml**: 매일 오후 2시 KST에 배포된 앱에 수집 실행 요청
